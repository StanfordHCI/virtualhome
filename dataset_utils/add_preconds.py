...
def get_preconds_script(script_lines):
    ...
    for i in range(len(content)):
        ...
        if action.upper() == 'SWITCHOFF':
            if obj_id not in is_on.keys():  # If this light was never switched on/off
                ...
                if not is_on[obj_id]:
                    pass
                    # zhuoyue: ignore turned off twice problem for now
                    # raise ScriptFail('Error, object turned off twice')
            is_on[obj_id] = False

        if action.upper() == 'SWITCHON':
            # zhuoyue: for naw, forget about turn on twice
        ...
        if action.upper() in ['RUN', 'WALK']:
            ...
            if object_id not in object_grabbed.keys() or not object_grabbed[object_id]:
                ...
                if last_walked is not None and last_walked[0].lower() in rooms:
                    pass
                    # zhuoyue: ignore this problem for now
    ...
    for i in range(len(content)):
        ...
        if obj_names[0] in rooms:
            last_room = (obj_names[0], ins_num[0])
        ...
        if action.upper() in ['WALK', 'RUN', 'FIND', 'LOOKAT']:
            if i + 1 < len(content):
                ...
                if len(obj_names2) > 0:
                    ...
                    newobj = [(obj_names2[it], ins_num2[it]) for it in range(len(obj_names2)) if
                              obj_names2[it].upper() not in rooms]
                    if len(newobj) > 0:
