...
def process_action(action, graph, env_graph):
    ...
    for pipeline_item in step_pipeline:
        ...
        for object_data in pipeline_item['objects']:
            ...
            items = [item for item in env_graph['nodes'] if
                     (item['class_name'] in name_equiv[object_type] or item['class_name'] == object_type)]
...
def render_script(comm, script, init_graph, scene_num, output, find_solution):
    ...
    s, home_capture_camera_ids = comm.home_capture_camera_ids() # zhuoyue: add camera here
